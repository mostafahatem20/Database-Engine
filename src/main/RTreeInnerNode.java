...
public class RTreeInnerNode extends RTreeNode implements Serializable {
	...
	protected RTreeLeafNode getFirstNode() {
		...
		return children[0].getFirstNode();
	}
	public RTreeNode getNodeGreater(Double key) {
		return children[findIndex(key)].getNodeGreater(key);
	}
	 ...
	 */
	private void insertAt(int index, Double  key) {
	 ...
	 *                  index
	 */
	public void insertLeftAt(int index, Double  key, RTreeNode  leftChild) {
		insertAt(index, key);
	 ...
	 *                   that index
	 */
	public void insertRightAt(int index, Double  key, RTreeNode  rightChild) {
		insertAt(index, key);
	...
	public boolean delete(Double key, RTreeInnerNode  parent, int ptr, int PageNum, int IndexInPage) {
		...
		for (int i = 0; !done && i < numberOfKeys; ++i)
			if (keys[i].compareTo(key) > 0)
				done = children[i].delete(key, this, i, PageNum, IndexInPage);

		if (!done)
			done = children[numberOfKeys].delete(key, this, numberOfKeys, PageNum, IndexInPage);
		if (numberOfKeys < this.minKeys()) {
			if (isRoot()) {
				this.getFirstChild().setRoot(true);
				this.setRoot(false);
	 ...
	 */
	public void merge(Double  parentKey, RTreeInnerNode  foreignNode) {
	...
	public Vector searchPos(Double key) {
		return children[findIndex(key)].searchPos(key);
	}

	public RTreeNode  getNode(Double key) {
		return children[findIndex(key)].getNode(key);
	}

	public void setRef(Double key, Vector<Ref> reference) {
		children[findIndex(key)].setRef(key, reference);
	}

	public void setRef2(Double key, Ref reference, int N) {

		int k = findIndex(key);
		children[k].setRef2(key, reference, N);
	}

	public void setRef3(Double key, Ref reference, int N) {
		if (numberOfKeys > 0)
			children[0].setRef3(key, reference, N);
	...
	protected boolean deleteByRef(Ref ref, RTreeInnerNode  parent, int ptr) {
		boolean done = false;
		for (int i = 0; !done && i < numberOfKeys; ++i)
			done = children[i].deleteByRef(ref, this, i);

		if (!done)
			done = children[numberOfKeys].deleteByRef(ref, this, numberOfKeys);
...
//		return done;
	...
	public void setRef4(Ref reference, int N) {
		if (numberOfKeys > 0)
			children[0].setRef4(reference, N);
	}

	public void setRef5(Ref reference, int N) {
		if (numberOfKeys > 0)
			children[0].setRef5(reference, N);

	}

	public void deletedPage(int PageNum) {
		if (numberOfKeys > 0)
			children[0].deletedPage(PageNum);
	}
